...
本矩阵只压缩 `source-ledger.md`、`solution-inventory.md` 和 `code-domain-linkage.md` 已登记的证据，不把“组件公开提供某项功能”外推为“该功能已在 WiFiDemo 上有效”。组件级事实与项目角色继续以 `solution-inventory.md` 为准；本文件只比较两个完整架构族，以及可附着在任一架构族上的 0/1 查询模式。
...
原矩阵的固定维度全部保留，但评估对象改为两个完整架构族。这里的状态描述架构是否容纳该职责，不是产品得分；凡涉及实际准确率、资源、Agent 效果或采用性，均保守留给 Benchmark。
...
| 6 断言与链接层 | 共同 assertion service | 同 A | `EXTRACTED`、`RULE_DERIVED`、`INFERRED_CANDIDATE`、`CURATED` 权限与 lifecycle 不因主干而改变 |
...
- **可证伪条件**：若 B01–B06/B11 的事实正确性低于 B，或 B10/B14 显示工具选择、跨 provider join、调用与延迟成本抵消模块化收益，则 A 的主张被削弱；若无法稳定绑定同一 snapshot/Target，则不满足完整架构硬门槛。
...
- **可证伪条件**：若 B01–B06/B11 显示 Target 泄漏、宏/occurrence/边错误，或 B10/B14 显示预构图成本、查询截断和运维负担高于其任务收益，则 B 的主张被削弱；若无法回到正确 revision/Target/file:line，则不满足完整架构硬门槛。
- **当前结论状态**：`unknown / benchmark required`。
...
2. **可定位源码证据**：事实与领域 assertion 回到 `repository_revision`、Target、文件、行范围与生成器；领域原始来源另回到 `source_revision_id` 与 locator [S025][S029]。
3. **不确定性治理**：规则、Agent、LLM、embedding 或聚类产生的软链接不能覆盖确定性代码事实，且必须保存 provenance、冲突、失效与重新验证 [S021–S025][S029]。

共同 assertion layer、source registry、snapshot consistency、许可证审计与 Agent evidence 双轴是共享条件，不能作为 A/B 胜负项。A/B 的核心差异只在程序事实主干与查询拓扑；0/1 的核心差异只在是否增加轻量发现后核验。
